...
export function TabStock() {
    ...
    const fetchBloodTypeFilteredData = async (selectedBlood, bbType, startDate, endDate) => {
        try {
    ...
    try {
        ...
        if (searchQuery) {
            ...
            response = await axios.get(`${laravelBaseUrl}/api/get-disposed-bloodbag`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                params: params,
    ...
    }, [router, sortColumn, sortOrder, searchQuery, blood_type, bbType, startDate, endDate]);
    ...
    const exportBloodBagsAsPDF = async () => {
        try {
            ...
            const response = await axios.get(`${laravelBaseUrl}/api/export-disposed-bloodbag`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
                responseType: "blob",
                        ...
                        <div className="flex items-center gap-4">
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    fetchBloodTypeFilteredData(blood_type, bbType, newStartDate, endDate);
                                }}
                                className=""
                            />
                            ...
                            <Input
                                ...
                                onChange={(e) => {
                                    ...
                                    fetchBloodTypeFilteredData(blood_type, bbType, startDate, newEndDate);
                                }}
                                className=""
                            />
                        </div>
                    ...
                    <tbody>
                        {userDetails.map((user, index) => (
                            <tr key={user.blood_bags_id} className={`${selectedRows.includes(user.blood_bags_id) ? selectedRowClass : ""}`}>
                                ...
                                <td className={classes}>
                                    <Typography variant="small" color="blue-gray" className="font-normal">
                                        {user.unsafe === 0 ? 'Expired' : user.unsafe === 1 ? 'Reactive' : user.unsafe === 2 ? 'Spoiled' : ''}
                                    </Typography>
                                </td>
