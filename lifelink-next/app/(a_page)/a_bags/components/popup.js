            ...
            <Dialog open={open} handler={() => setOpen(false)}>
                ...
                <DialogBody divider className="flex flex-col gap-4">
                    <div>
                        ...
                        {errorMessage.serial_no && <div className="error-message text-red-600 text-sm mt-1">{errorMessage.serial_no}</div>}
                    </div>
                    <div className={`relative ${errorMessage.bled_by ? "mb-1" : ""}`}>
                        <Input label="Bled by" value={bledBy} disabled={countdown === 0} onChange={(e) => setBledBy(e.target.value)} />
                        {errorMessage.bled_by && <div className="error-message text-red-600 text-sm">{errorMessage.bled_by}</div>}
                    </div>
                    <div className={`relative ${errorMessage.venue ? "mb-1" : ""}`}>
                        <Input label="Venue" value={venue} disabled={countdown === 0} onChange={(e) => setVenue(e.target.value)} />
                        {errorMessage.venue && <div className="error-message text-red-600 text-sm">{errorMessage.venue}</div>}
                    </div>
                    <div className={`relative ${errorMessage.date_donated ? "mb-1" : ""}`}>
                        <Input type="date" label="Date" value={dateDonated} disabled={countdown === 0} onChange={(e) => setDateDonated(e.target.value)} max={new Date().toISOString().split("T")[0]} />
                        {errorMessage.date_donated && <div className="error-message text-red-600 text-sm">{errorMessage.date_donated}</div>}
                    </div>
                </DialogBody>
                <DialogFooter>
