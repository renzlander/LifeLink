...
export function ManPowerTable({ manPowerCount, manPowerList }) {
  ...
  const chunkedNames = Array.from(
    { length: Math.ceil(manPowerList.length / numberOfColumns) },
    (_, rowIndex) => {
      const rowNames = manPowerList
        .slice(rowIndex * numberOfColumns, (rowIndex + 1) * numberOfColumns)
        .map((item) => item || "");

      // Add blank columns if there are 1 or 2 names in the row
      if (rowNames.length < numberOfColumns) {
        const blankColumns = Array.from(
          { length: numberOfColumns - rowNames.length },
          (_, index) => ""
        );
