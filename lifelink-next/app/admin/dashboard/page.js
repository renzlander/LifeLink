...
import { BloodListCard, BloodQuota, CountDonorCard } from "./components/Cards";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${laravelBaseUrl}/api/dashboard-get-stocks`,
          {
            headers: {
    ...
    const fetchMBDSummary = async () => {
      try {
        ...
        const mbdSummary = await axios.get(
          `${laravelBaseUrl}/api/dashboard-mbd-quick-view`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const fetchQuota = async () => {
      try {
        ...
        const response = await axios.get(
          `${laravelBaseUrl}/api/dashboard-get-quota`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        <Card className="mt-6 w-full bg-gray-100">
          ...
          <CardFooter className="flex justify-end items-center gap-4 pt-0 pb-2 pr-6">
            <div className="flex items-center gap-1">
              ...
              <Typography className="text-sm text-gray-700 font-normal">
                Last Updated:
              </Typography>
            </div>
            <Typography className="text-gray-700 text-md font-normal">
              {formatDate(updatedAt)} {updatedAtTime}
            </Typography>
          </CardFooter>
        </Card>
