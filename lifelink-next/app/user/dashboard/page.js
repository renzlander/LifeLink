  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${laravelBaseUrl}/api/get-available-blood`,
          {
            headers: {
        ...
        if (response.data && Array.isArray(response.data.blood_bags)) {
          const bloodTypes = response.data.blood_bags.map(
            (bag) => bag.blood_type
          );
          const availability = response.data.blood_bags.map(
            (bag) => bag.status
          );
    ...
    const fetchRecentPost = async () => {
      try {
        ...
        const response = await axios.get(
          `${laravelBaseUrl}/api/get-recent-post`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    const fetchMbdSummary = async () => {
      try {
        ...
        const response = await axios.get(
          `${laravelBaseUrl}/api/get-donation-summary`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
      ...
      <div className="flex 3xl:flex-row flex-col justify-between gap-4 w-full">
        <Card className="mt-6 w-full bg-gray-100">
          ...
          <CardFooter className="flex justify-end items-center gap-4 pt-0 pb-2 pr-6">
            ...
            <Typography className="text-gray-700 text-xl font-normal">
